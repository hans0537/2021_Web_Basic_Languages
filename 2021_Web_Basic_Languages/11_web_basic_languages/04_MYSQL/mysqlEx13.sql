...
FROM
		DEPARTMENTS AS D,
        DEPT_EMP DE		# AS는 생략 가능하다, 보통 생략한다.
WHERE
...
SELECT
		# (참고) 자바의 변수와 매칭하는 것 외에 관리적 차원으로도 AS 를 사용한다.
        # DE.EMP_NO		AS EMP_NO,	
        # D.DEMP_NO		   DEMP_NO,	
        # D.DEPT_NAME	   DEPT_NAME
        
		DE.EMP_NO		AS empNo,	 # 자바 영역으로 보내줄 key 변수를 지정해준다. select된 value값이 key에 저장되어 보내진다.
        D.DEPT_NO		   deptNo,	 # AS 는 생략 가능하다.	
...
# INNER JOIN , LEFT JOIN , girl_groupRIGHT JOIN 테스트를 위한 데이터 셋업
...
# GIRL_GROUP이 왼쪽 SONG이 오른쪽 기준으로 같은 ROW 조회후 나머지 왼쪽 정보 다 나옴
...
FROM
		GIRL_GROUP GG
   LEFT OUTER JOIN SONG S					# OUTER는 생략이 가능하다.
				ON S._ID = GG.HIT_SONG_ID;
          
# 예시 3) RIGHT JOIN : SONG의 데이터는 모두 조회되고 ID와 HIT_SONG_ID가 같은 row를 조회
# GIRL_GROUP이 왼쪽 SONG이 오른쪽 기준으로 같은 ROW 조회후 나머지 오른쪽 정보 다 나옴
...
FROM
		GIRL_GROUP GG
   RIGHT OUTER JOIN SONG S					# OUTER는 생략이 가능하다.
				ON S._ID = GG.HIT_SONG_ID;
